...
public class MiniFillerCommand extends CoreCommandPlus {
    public MiniFillerCommand() {
        super("minifiller", Minigames.get(), Perms.COMMAND_MINIFILLER);
        addSubCommandHandler("create", (sender, label, args) -> {
            ...
            if (args.length != 2) {
                //arguments
                MessageUtil.sendMessage(sender, "minifiller.error.create_arguments_amount",
                        "%label%", label);
                return;
            }
            String id = args[1].toLowerCase();
            if (!UtilsString.isLowcasedValidID(id)) {
                MessageUtil.sendMessage(sender, "minifiller.error.invalid_id",
                        "%id%", args[1]);
                return;
            }
            if (FillerManager.get().getFiller(id) != null) {
                MessageUtil.sendMessage(sender, "minifiller.error.already_used_id",
                        "%id%", args[1]);
                return;
            }
            ChestFiller filler = new ChestFiller();
            FillerManager.get().registerFiller(id, filler, (Player) sender);
            FillerManager.get().save(filler);
            MessageUtil.sendMessage(sender, "minifiller.success.create",
                    "%id%", id);
        }, null);
        addSubCommandHandler("delete", (sender, label, args) -> {
                    if (args.length != 2) {
                ...
                , (sender, label, args) -> args.length == 2 ? this.complete(args[1], FillerManager.get().getFillers().keySet())
                        : Collections.emptyList());
        addSubCommandHandler("addall", (sender, label, args) -> {
            ...
            MessageUtil.sendMessage(player, "minifiller.success.addall",
                ...
                args.length == 2 ? this.complete(args[1], FillerManager.get().getFillers().keySet()) :
                        args.length == 3 ? this.complete(args[2], List.of("10", "50", "100")) : Collections.emptyList());
        addSubCommandHandler("show", (sender, label, args) -> {
            if (!(sender instanceof Player player)) {
                playerOnlyNotify(sender);
                return;
            }
        ...
        }, (sender, label, args) -> args.length == 2 ? this.complete(args[1], FillerManager.get().getFillers().keySet()) : Collections.emptyList());
        addSubCommandHandler("list", new SubCommandListExecutor<>(this, "list",
                () -> new TreeSet<>(FillerManager.get().getFillers().keySet()),
                (fillerId) -> fillerId,
                (fillerId, sender) -> String.join("\n", MessageUtil.getMultiMessage(sender, "minifiller.success.list_hover", "%id%", fillerId))
        ), null);
        addSubCommandHandler("show", (sender, label, args) -> {
            if (!(sender instanceof Player player)) {
                playerOnlyNotify(sender);
